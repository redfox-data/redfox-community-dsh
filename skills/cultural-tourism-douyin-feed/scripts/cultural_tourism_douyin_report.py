...
文旅抖音信息源 — 搜索文旅热门作品
=====================================
根据关键词搜索文旅抖音热门作品，自动聚类分类展示，生成 HTML 报告。

Usage:
    python3 cultural_tourism_douyin_report.py --keyword "九寨沟"
    python3 cultural_tourism_douyin_report.py --keyword "张家界" --start-time "2026-06-01"
    python3 cultural_tourism_douyin_report.py --keyword "丽江" --subscribe
"""
...
# ─── 配置 ─────────────────────────────────────────────────────────────────────────
API_URL = "https://redfox.hk/story/api/parseWork/queryDyPlayletMsgs"
CONFIG_DIR = Path.home() / ".qoder" / "apis"
CONFIG_FILE = CONFIG_DIR / "redfox.json"
ENV_KEY = "REDFOX_API_KEY"
SOURCE = "文旅抖音信息源-GitHub"

PAGE_SIZE = 200

DEFAULT_OUTPUT_DIR = Path.home() / "Downloads" / "QoderReports"
PLIST_LABEL = "com.qoder.cultural-tourism-douyin-feed"
...
def get_work_url(article):
    """获取作品可点击链接，优先使用 url 字段，fallback 用 photoId 构建"""
    url = article.get("url") or ""
    if not url:
        photo_id = article.get("photoId") or article.get("workId") or ""
        if photo_id:
            url = f"https://www.iesdouyin.com/share/video/{photo_id}"
    return url
...
def print_article_table(clusters, keyword):
    ...
    print(f"\n{BOLD}{'='*78}{RESET}")
    print(f"{BOLD}  文旅抖音信息源 · {kw_label}分类作品{RESET}")
...
def get_fallback_template():
    return (
        ...
        '.stat-value { font-size:1.5rem; font-weight:bold; color:#25F4EE; }'
        ...
        '.card-header { display:flex; align-items:center; gap:0.8rem; padding:1rem 1.2rem; background:linear-gradient(135deg,#25F4EE,#5CF7F2); }'
        ...
        '.article-title:hover { color:#25F4EE; }'
        ...
        '<div class="footer">Generated at {{TIMESTAMP}} by 文旅抖音信息源 Skill</div>'
        '</body></html>'
    )
...
def main():
    ...
    if not articles:
        error(f"未获取到「{keyword}」相关作品")
        print(f"\n{YELLOW}  提示：换个关键词试试，或使用更通用的词。{RESET}")
        sys.exit(1)

    info(f"获取完成: {len(articles)} 个作品")
    ...
    try:
        ...
        file_date = effective_start
    filename = f"文旅抖音日报_{safe_keyword}_{file_date}.html"
