#!/usr/bin/env python3
"""
Instagram视频下载 - API 版本
使用 redfox.hk API 解析 Instagram 视频链接，直接返回无水印视频下载链接
...
def main():
    parser = argparse.ArgumentParser(
        description="Instagram视频下载 - 使用 redfox.hk API 解析视频并返回下载链接",
...
也可通过环境变量 REDFOX_API_KEY 配置密钥：
  ...
  python3 downloader.py <url>
    ...
    parser.add_argument("url", help="Instagram 视频链接，如 https://www.instagram.com/reel/xxxxx/")
...
"""
    ...
    try:
        resp = session.post(API_URL, json={"url": url, "source": "ins视频下载-GitHub"}, timeout=30)
        result = resp.json()
