...
description: 豆包 WebSearch 搜索工具。基于红狐 API 调用豆包联网搜索能力，提交查询后自动轮询等待结果返回。当用户需要使用豆包搜索、联网搜索、AI 搜索获取实时信息时使用。触发词：豆包搜索、豆包websearch、联网搜索、AI搜索、doubao搜索。
---
...
通过红狐 API 调用豆包联网搜索能力，提交查询后自动轮询等待结果返回。适用于需要联网实时搜索、AI 辅助信息检索的场景。
...
脚本自动完成：
1. **提交搜索**：`POST /story/api/doubaoSearch/submit`，传入 `{"inquiry_text": "...", "source": "豆包websearch-GitHub"}`
2. **轮询结果**：每 5 秒 `POST /story/api/doubaoSearch/result` 查询一次，直到状态为 `completed`
3. **实时反馈**：轮询期间通过 stderr 输出等待提示
4. **最终输出**：状态 `completed` 后输出完整 JSON 结果到 stdout

### Step 3：展示结果

脚本返回 JSON 后，提取 `data.content` 中的豆包回答文本和 `data.result` 中的引用来源，结构化整理后向用户展示。
