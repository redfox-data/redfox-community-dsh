...
文旅B站信息源 — 搜索文旅热门视频
=====================================
根据关键词搜索文旅B站热门视频，自动聚类分类展示，生成 HTML 报告。

Usage:
    python3 cultural_tourism_bilibili_report.py --keyword "九寨沟"
    python3 cultural_tourism_bilibili_report.py --keyword "张家界" --start-time "2026-06-01"
    python3 cultural_tourism_bilibili_report.py --keyword "丽江" --subscribe
"""
...
# ─── 配置 ─────────────────────────────────────────────────────────────────────────
API_URL = "https://redfox.hk/story/api/parseWork/queryBiliPlayletMsgs"
CONFIG_DIR = Path.home() / ".qoder" / "apis"
CONFIG_FILE = CONFIG_DIR / "redfox.json"
ENV_KEY = "REDFOX_API_KEY"
SOURCE = "文旅B站信息源-GitHub"
...
def print_article_table(clusters, keyword):
    ...
    print(f"\n{BOLD}{'='*78}{RESET}")
    print(f"{BOLD}  文旅B站信息源 · {kw_label}分类作品{RESET}")
...
def get_fallback_template():
    return (
        ...
        '.card-header { display:flex; align-items:center; gap:0.8rem; padding:1rem 1.2rem; background:linear-gradient(135deg,#FB7299,#FF8DB3); }'
        ...
        '<div class="footer">Generated at {{TIMESTAMP}} by 文旅B站信息源 Skill</div>'
        '</body></html>'
    )
...
<dict>
    ...
    <false/>{env_section}
        ...
        try:
            ...
            info(f"订阅成功! 每天 09:00 自动生成「{keyword}」文旅B站日报")
...
def main():
    ...
    if not articles:
        error(f"未获取到「{keyword}」相关视频")
        print(f"\n{YELLOW}  提示：换个关键词试试，或使用更通用的词。{RESET}")
        sys.exit(1)

    info(f"获取完成: {len(articles)} 个视频")
    ...
    try:
        ...
        file_date = effective_start
    filename = f"文旅B站日报_{safe_keyword}_{file_date}.html"
