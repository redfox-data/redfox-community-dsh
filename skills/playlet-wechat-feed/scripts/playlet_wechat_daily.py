...
公众号平台适配（相对抖音版）：
- platform=0（公众号）
- 排序与展示以阅读量 readCount 为核心指标
- 账号字段多字段兜底 verifyName / accountName / author
- 文章链接多字段兜底 url / workUrl / articleUrl
- 主题色 #07C160（微信绿）

用法与原版完全兼容：
    python3 playlet_wechat_daily.py --latest
    python3 playlet_wechat_daily.py --date 2026-08-05
    python3 playlet_wechat_daily.py --topics "穿越,霸总" --latest
"""
...
def generate_html_report(items, clusters, date_str):
    """生成HTML日报（微信绿 #07C160，阅读/点赞/评论零值隐藏）"""
