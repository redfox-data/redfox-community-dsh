...
def get_api_key():
    """获取 API Key：环境变量优先，其次扫描 Shell 配置文件"""
    ...
    for cfg in SHELL_CONFIG_FILES:
        ...
        if cfg_path.is_file():
            try:
                ...
                continue
    print("❌ 错误：未找到 REDFOX_API_KEY。", file=sys.stderr)
    print("请配置环境变量，或将 export REDFOX_API_KEY=ak_xxxxxxxx 写入 ~/.zshrc / ~/.bashrc 后 source：", file=sys.stderr)
    print("  API Key 获取：https://redfox.hk/?source=github 注册登录后在个人中心获取", file=sys.stderr)
    sys.exit(1)
