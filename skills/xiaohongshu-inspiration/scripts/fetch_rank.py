...
SHELL_CONFIG_FILES = [".zshrc", ".bashrc", ".bash_profile", ".profile", ".zprofile"]


def _get_api_key() -> str:
    """获取 API Key：环境变量优先，其次扫描 Shell 配置文件，返回格式 ak_xxxxxxxx。"""
    val = os.environ.get("REDFOX_API_KEY", "").strip()
    if val:
        return val
    for cfg in SHELL_CONFIG_FILES:
        cfg_path = Path.home() / cfg
        if cfg_path.is_file():
            try:
                content = cfg_path.read_text(encoding="utf-8", errors="replace")
                m = re.search(r'export\s+REDFOX_API_KEY\s*=\s*["\']?([a-zA-Z0-9_\-]+)["\']?', content)
...
def _http_post(url: str, payload: dict, timeout: int = 15) -> dict:
    ...
    if not api_key:
        print("[ERROR] 未找到 REDFOX_API_KEY，请设置环境变量或将 export REDFOX_API_KEY=ak_xxxxxxxx 写入 ~/.zshrc / ~/.bashrc 后 source", file=sys.stderr)
        print("[提示] API Key 获取：https://redfox.hk/?source=github 注册登录后在个人中心获取", file=sys.stderr)
        sys.exit(4)
...
def main():
    ...
    # 生成 HTML 报告文件
    if args.html:
        from generate_rank_report import generate_html
        # 准备数据
        html_data = {
