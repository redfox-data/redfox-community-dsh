...
**正负面占比口径**（报告摘要、品牌指纹、竞品表统一使用此口径）:
- 分母 = **全部有效回答数**（`status == "completed"`，平台限流/故障话术已标记为 `invalid` 并剔除）
- **正面率 = positive / 有效回答数 × 100%**
- **负面率 = negative / 有效回答数 × 100%**
- 注意：`正面率 ≠ 100% - 负面率`，中性评价单独计数，不可并入正面
