...
# ─── 常量 ──────────────────────────────────────────────────────────────────────────
VERSION = "v2.0.0"  # 与 SKILL.md 徽章、references/cn30-output-rules.md 保持一致
...
# ─── 引擎页脚 ───────────────────────────────────────────────────────────────────────
def build_engine_footer(platforms: dict) -> str:
    """构造引擎页脚，供报告逐字粘贴

    references/cn30-output-rules.md 的 LAW 1 / LAW 4 要求输出中包含
    「🇨🇳 数据源: …」页脚并逐字粘贴。该页脚由本函数生成，
    通过 summary.engine_footer 字段（stdout JSON）暴露给调用方。

    Args:
        platforms: data["platforms"] 结构

    Returns:
        str: 单行页脚，如 "🇨🇳 数据源: 小红书 · 抖音 · 公众号 · 近30天 · cn-last30days v2.0.0"
    """
    labels = [v.get("label", k) for k, v in platforms.items() if not v.get("error")]
    if not labels:
        labels = [v.get("label", k) for k, v in platforms.items()]
    total = sum(v.get("total", 0) for v in platforms.values())
    return (
        f"🇨🇳 数据源: {' · '.join(labels)} · 近30天 · "
        f"共 {total} 条 · cn-last30days {VERSION}"
    )
...
def main():
    ...
    summary = {
        ...
        # 引擎页脚：供报告逐字粘贴（references/cn30-output-rules.md LAW 4）
        "engine_footer": build_engine_footer(data["platforms"]),
        "files": {},
    }
    if json_file:
