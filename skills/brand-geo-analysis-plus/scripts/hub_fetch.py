...
# 平台展示固定顺序（与 SKILL.md 输出规范一致）
# 百度 → 知乎 → 微博 → 抖音 → B站 → 快手 → 头条
DISPLAY_ORDER = ["bd", "zh", "wb", "dy", "bz", "ks", "tt"]
...
def ordered_platforms(result: Dict):
    """按固定展示顺序返回 (platform_code, platform_data) 列表

    API 返回顺序不稳定（实测为 百度/B站/抖音/快手/头条/微博/知乎），
    这里统一重排为 SKILL.md 规定的顺序；未知平台码追加在末尾。
    """
    platforms = result.get("platforms", {})
    ordered = [(c, platforms[c]) for c in DISPLAY_ORDER if c in platforms]
    ordered += [(c, d) for c, d in platforms.items() if c not in DISPLAY_ORDER]
    return ordered
...
def output_compact(result: Dict) -> str:
    ...
    # 各平台概览（每平台仅预览 TOP3，完整数据见 dataFile）
    for platform_code, platform_data in ordered_platforms(result):
        total_n = platform_data["count"]
        preview_n = min(3, total_n)
        output_lines.append(f"[{platform_data['name']}] {total_n}条（下表为 TOP{preview_n} 预览）")
