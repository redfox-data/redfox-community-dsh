...
def _generate_summary(analysis, platforms, total_completed, total_mentioned, position):
    ...
    # --- 情绪口径（与报告「情感分析」章节保持一致：分母为全部有效回答）---
    if total_sent > 0:
        pos_pct = _pct(pos_count, total_sent)
        neu_pct = _pct(neu_count, total_sent)
        neg_pct = _pct(neg_count, total_sent)
        sentiment_text = (
            f"正面率 {pos_pct}%（{pos_count}正 / {neu_count}中 / {neg_count}负）"
    ...
    <div class="summary">
      ...
      <div class="summary-bd">
        <p><strong>综合评估:</strong> {brand} 跨平台GEO综合得分 <strong>{geo_o}</strong> 分，{grade_text}。品牌提及率 {mr_pct}%（{total_mentioned}/{total_completed} 条有效回答提及），{sentiment_text}。整体品牌认知度{'较高' if mr_pct >= 50 else '有待提升'}。{coverage_note}</p>
...
def _render_fingerprint(analysis, platforms, total_completed, total_mentioned, position):
    ...
    pos_pct = _pct(sentiment_dist.get("positive", 0), total_sent) if total_sent else 0
...
def _render_sentiment_merged(analysis, platforms, has_ai, total_completed, pos_tags, neg_tags, brand):
    ...
    pos_pct = _pct(overall.get("positive", 0), total) if total else 0
...
def _render_competitors(analysis, platforms, total_completed):
    ...
    brand_pos_pct = _pct(brand_sentiment.get("positive", 0), brand_total_s) if brand_total_s else 0
    ...
    for comp in sorted_comps:
        ...
        pp = _pct(sd.get("positive", 0), sd_total) if sd_total else 0
        detail = ""
        if sd:
