...
def generate_html_report(items, clusters, date_str):
    ...
    for i, (topic, topic_items) in enumerate(sorted(clusters.items(), key=lambda x: len(x[1]), reverse=True), 1):
        ...
        for item in topic_items[:5]:  # 每个题材展示前5个
            ...
            url = item.get("url") or ""  # 作品链接
            photo_id = item.get("photoId") or ""  # 作品ID
            likes = format_number(item.get("likeCount", 0))
            comments = format_number(item.get("commentCount", 0))  # 评论数
            ...
            # 生成作品链接:优先使用url字段,如果为null则用photoId拼接
            if url:
...
def main():
    parser = argparse.ArgumentParser(description="短剧-抖音信息源日报生成工具")
