...
- **失败阈值限制**：同一小红书号参数6小时内失败3次后，脚本返回 `query_type: "threshold_exceeded"`
  - 告知用户：「当前小红书号已超过失败阈值，请联系客服邮箱redfoxdata@proton.me 或访问 https://redfox.hk/?source=coze处理」

**⚠️ 账号数据为空处理（未查询到账号）**：
- 如果脚本返回 `query_type: "not_found"`，说明该账号不在数据库中
- **立即停止后续步骤**，输出以下提示并结束本次分析：
  ```
  未查询到相关账号：当前 Skill 仅收录热门账号。如需定制数据，可邮件联系[红狐数据](https://redfox.hk/?source=coze)咨询：redfoxdata@proton.me
  ```
