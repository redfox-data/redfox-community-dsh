...
> **重要**：数据每日15:00更新前一天数据（实际可能延迟，以脚本真实探活为准）。查询前脚本先做输入校验：分类/关键词不符合短剧题材词库时**不请求任何接口**，直接提醒"关键词不满足查询条件"并推荐相关词；日期超出有效查询范围时提醒并自动回退最近有数据日期，无需用户确认。
...
- **🧭 前置输入校验**：查询前先判断用户的分类/关键词是否符合短剧题材词库、日期是否在有效查询范围；全部不满足时**不请求任何接口**，直接提醒"关键词不满足查询条件"并推荐相关分类和关键词；混合词保留有效关键词查询
...
├── scripts/
│   └── playlet_bili_daily.py        # 核心脚本（v2.3增强版：前置校验零请求/推荐/探活/自动回退/扩展题材/兜底）
└── references/
...
```
用户查询 → 前置输入校验(分类/关键词/日期) → 日期预检(真实探活) → API调用(queryPlayletMsgs) → 数据去重 → 题材聚类 → HTML日报生成 → 浏览器打开
              ↓(关键词/分类全部不满足)            ↓(日期未更新/超范围)       ↓(数据不足)           ↓(关键词无匹配)
       不请求接口,直接提示+推荐停止        自动回退最近有数据日期并告知  自动扩展题材(穿越→霸总→重生→悬疑→甜宠→逆袭)  自动降级全量查询并告知
                                              ↓
                                    终端输出趋势分析报告
```
