...
# 兜底基准数据（历史遗留占位，当前评分流程由 API 返回 accountAvgList/accountExcellentList）
BENCHMARK_DATA = {}
...
def _get_credential():
    ...
    raise ValueError(
        ...
        "获取API Key: 访问 https://redfox.hk/?source=github 注册后在个人中心获取"
    )
...
def _analyze_single_account(raw, has_works=True):
    ...
    # 综合评分：四维度加权均值（内容健康30% + 用户活跃30% + 核心数据30% + 运营规范性10%）
