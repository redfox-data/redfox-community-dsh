...
> 🔴 **链接禁止截断：`标题`字段已包含完整的 Markdown 链接格式 `[标题文字](URL)`，输出时必须直接使用该字段完整内容，禁止用 `(…)` 或任意内容替换 URL 部分；可适度截短显示的标题文字，但完整链接地址必须保留。**
...
> *诊断时间：[YYYY-MM-DD] | 数据来源：[红狐数据（redfox.hk）](https://redfox.hk/?source=github) 更多平台数据、优质数据可前往查看API接口 | 置信度：[高/中/低] | 阅读数>10万显示为10w+*
