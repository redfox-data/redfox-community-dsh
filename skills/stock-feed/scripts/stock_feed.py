...
Usage:
    python stock_feed.py
    python stock_feed.py --days 30
    python stock_feed.py --keyword "半导体,芯片"
    python stock_feed.py --output-format html
"""
...
# 强制 stdout/stderr UTF-8 编码（兼容所有平台）
...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .report-content strong {{ color: #7f1d1d; }}
...
def main():
    ...
    parser.add_argument(
        ...
        help="搜索关键词（默认使用A股17个核心关键词，--from-json 模式下可省略）"
    )
    parser.add_argument(
        "--platforms", "-p",
        default="xhs,dy,gzh",
    ...
    parser.add_argument(
        ...
        help=f"搜索时间范围，最近多少天（默认: {DEFAULT_DAYS}，最大: 30）"
    )
    parser.add_argument(
        "--output-format", "-f",
    ...
    parser.add_argument(
        "--output-dir",
        default=str(Path.home() / "Downloads" / "StockFeed"),
        help="HTML 输出目录"
    )
    parser.add_argument(
        "--api-key",
    ...
    base_name = f"stock_feed_{keyword_safe}_{timestamp}"
    ...
    # stdout 输出精简摘要（关键词截断避免终端截断）
    keyword_display = keyword if len(keyword) <= 40 else keyword[:40] + f"...(共{len(keyword.split(','))}词)"
    summary = {
