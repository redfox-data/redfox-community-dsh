#!/usr/bin/env python3
"""
Kimi WebSearch — 提交搜索查询并轮询结果

用法:
    python3 kimi_search.py "<搜索关键词>" [source标识]
...
API_BASE = "https://redfox.hk/story/api/kimi"
...
def main():
    ...
    query = sys.argv[1]
    source = sys.argv[2] if len(sys.argv) > 2 else "kimi websearch-GitHub"
    ...
    print(
        f"⏳ 已提交搜索请求 (taskId: {task_id})，正在等待 Kimi WebSearch 返回结果…",
        file=sys.stderr,
    )
