...
| 参数 | 说明 |
|------|------|
| `--account` | 抖音昵称或抖音号（统一使用 `uniqueName` 参数查询） |
| `--output` | 输出格式：`markdown`（默认）/ `json` |

**自动识别逻辑：**
- 所有输入统一使用 `uniqueName` 参数查询
- 中文昵称为非唯一标识，检测到中文输入时直接阻断，不发起 API 调用，输出引导提示：

```
⚠️ 检测到中文昵称输入：「{输入}」
   中文昵称不唯一，暂不支持昵称查询，请使用抖音号/抖音ID查询。
   抖音号类型示例：
     - 纯字母：luoyonghao
     - 字母+数字：18561019369wn
     - 纯数字：50734407794
   抖音号查看方式：抖音APP → 目标账号主页 → 昵称下方
```
